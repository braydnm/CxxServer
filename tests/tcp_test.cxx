...
namespace {
    ...
    using Session = CxxServer::Core::Tcp::Session;
    using Server = CxxServer::Core::Tcp::Server;
    using Client = CxxServer::Core::Tcp::Client;
    ...
    class EchoServer : public Server {
        ...
        protected:
            std::shared_ptr<Session> newSession(const std::shared_ptr<Server> &server) override { return std::make_shared<EchoSession>(server); }

            void onStart() override { started = true; }
            void onStop() override { stopped = true; }
            void onConnect(std::shared_ptr<Session> &s) override { connected = true; ++connections; }
            void onDisconnect(std::shared_ptr<Session> &s) override { disconnected = true; --connections; }
            void onErr(int error, const std::string &category, const std::string &message) override { errors = true; }
    };
    ...
    TEST_CASE("TCP server test", "[CxxServer][TCP]") {
        ...
        while (!client->isConnected() || (server->connections != 1))
            std::this_thread::yield();
    ...
    TEST_CASE("TCP multicast server test", "[CxxServer][TCP]") {
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            while (!clients.back().first->isConnected() || server->connections != clients.size())
                std::this_thread::yield();
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            while (client.first->isConnected() || server->connections != clients.size() - i - 1)
                std::this_thread::yield();
    ...
    TEST_CASE("TCP random stress test", "[CxxServer][TCP]") {
        ...
        while (std::chrono::duration_cast<std::chrono::seconds>(std::chrono::high_resolution_clock::now() - start).count() < 10) {
            if ((rand() % 1000) == 0) {
                ...
                while (!clients.back()->isConnected())
                    std::this_thread::yield();
            }
            else if ((rand() % 100) == 0 && !clients.empty()) {
                size_t idx = rand() % clients.size();
                bool state = clients[idx]->isConnected();
                ...
                while (clients[idx]->isConnected() == state)
                    std::this_thread::yield();
            }
            else if ((rand() % 100) == 0 && !clients.empty()) {
                size_t idx = rand() % clients.size();
